...
curl -sfL https://get.k3s.io | K3S_KUBECONFIG_MODE="777" INSTALL_K3S_VERSION="v1.21.14+k3s1" K3S_TOKEN=SECRET INSTALL_K3S_EXEC="--node-ip=${IPADDR} --flannel-backend=none --cluster-cidr=192.168.8.0/16 --disable-network-policy --disable=traefik" sh -s - server --cluster-init --tls-san "$(cat /vagrant/LB_IP)" --disable servicelb

#curl -sfL https://get.k3s.io | K3S_KUBECONFIG_MODE="777" INSTALL_K3S_VERSION="v1.21.14+k3s1" K3S_TOKEN=SECRET INSTALL_K3S_EXEC="--node-ip=${IPADDR} --cluster-cidr=192.168.8.0/16 --disable=traefik" sh -s - server --cluster-init --tls-san "$(cat /vagrant/LB_IP)" --disable servicelb
