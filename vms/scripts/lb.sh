...
do
    server_list="${server_list}    server master-0${ci}.local:6443;\n"
	ci=$((ci+1))
done
...
sudo echo "* * * * * root if ping -c 1 master-01.local; then sudo /etc/init.d/nginx reload; sudo echo \"*/5 * * * * root update_masters.sh\" > /etc/cron.d/watchmaster; sudo rm -f /etc/cron.d/connect; fi" > /etc/cron.d/connect
